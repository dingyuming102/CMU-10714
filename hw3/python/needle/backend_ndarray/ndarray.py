...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if prod(new_shape) != prod(self._shape):
            raise ValueError("New shape is NOT compatible with the current shape!")
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        # 在被除数上+除数-1是整除向上取整的常用方法。原理是只要有余数，结果就会+1，如果没有余数，则不会+1.
        ...
        offset = builtins.sum( [stride*sl.start for sl, stride in zip(idxs, self.strides)] )
        return NDArray.make(new_shape, new_strides, self.device, self._handle, offset)
        ### END YOUR SOLUTION

...
def softmax_loss(Z, y_one_hot):
    ...
    """
    ### BEGIN YOUR SOLUTION
    hy_term = (Z * y_one_hot).sum()
    log_sum_exp_h_term = ndl.log( ndl.exp(Z).sum(axes=(1, )) ).sum()
    return (log_sum_exp_h_term - hy_term) / Z.shape[0]
    ### END YOUR SOLUTION
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for i in range(iteration_num):
        ...
        Z1, y_b = ndl.Tensor(X[start_idx: end_idx, :]), y[start_idx: end_idx]
        
        Z3 = ndl.relu(Z1 @ W1) @ W2
        
        # 不能用zeros_like,因为Z3不是ndarray类型，不兼容
        y_b_one_hot = np.zeros(Z3.shape)
        y_b_one_hot[np.arange(m), y_b] = 1
        y_b_one_hot = ndl.Tensor(y_b_one_hot)
        
        loss = softmax_loss(Z3, y_b_one_hot)
        loss.backward()
        
        W1 = (W1 - lr * W1.grad).data
        W2 = (W2 - lr * W2.grad).data

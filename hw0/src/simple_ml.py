...
def softmax_loss(Z, y):
    ...
    """
    ### BEGIN YOUR CODE
    exp_of_logits = np.exp(Z)
    sum_of_exps = np.sum(exp_of_logits, axis=1)
    log_of_sum = np.log(sum_of_exps)
    batchsum_of_log_term = np.sum(log_of_sum)
    
    batchsum_of_correct_class_logits = np.sum(Z[np.arange(y.size), y])
    
    batchsum_of_softmaxlosses = batchsum_of_log_term - batchsum_of_correct_class_logits
    average_loss = batchsum_of_softmaxlosses / y.size
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    # iteration_num = (num_examples + batch - 1) // batch
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(iteration_num):
        ...
        Z1, y_b = X[start_idx: end_idx, :], y[start_idx: end_idx]
        
        Z2 = Z1 @ W1
        # A1 = np.maximum(Z1, 0) 也可以，但不是就地修改
        Z2[Z2 < 0] = 0
        Z3 = Z2 @ W2
        
        G3 = np.exp(Z3)
        G3 /= G3.sum(axis=1, keepdims=True)
        Iy = np.zeros_like(G3)
        Iy[np.arange(m), y_b] = 1
        G3 -= Iy
        
        dsigma1 = np.zeros_like(Z2)
        dsigma1[Z2 > 0] = 1
        G2 = G3 @ W2.T

...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        if node.is_leaf():
            continue
        # compute partial adjoints for each input node
        partial_adjoints_tuple = node.op.gradient_as_tuple(node.grad, node)
        for prev_node, partial_adjoint in zip(node.inputs, partial_adjoints_tuple):
            if prev_node not in node_to_output_grads_list:
                node_to_output_grads_list[prev_node] = []
            node_to_output_grads_list[prev_node].append(partial_adjoint)
    ### END YOUR SOLUTION
